...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    struct sfs_disk_inode *din = sin->din;//内存中的 inode（sin）获取对应的磁盘 inode 结构体指针
    assert(din->type != SFS_TYPE_DIR);//确保不是目录类型
    off_t endpos = offset + *alenp, blkoff;
    //cprintf("want:%d,",*alenp);
    *alenp = 0;
    ...
    size_t size, alen = 0;
    uint32_t ino;
    uint32_t blkno = offset / SFS_BLKSIZE;          // 读写起始位置对应的逻辑块编号
    uint32_t nblks = endpos / SFS_BLKSIZE - blkno;  // 计算需要读写的完整块数量
    blkoff = offset % SFS_BLKSIZE;                  // 块内偏移
     //LAB8:EXERCISE1 2311656 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    //处理中间完整块
    uint32_t full_block_size = SFS_BLKSIZE;
    while(nblks > 0) {
        //获取当前逻辑块对应的物理块号
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) goto out;
        //传入物理块号ino，且每次只处理1个完整块
        if ((ret = sfs_block_op(sfs, buf, ino, 1)) != 0) goto out;
        //更新统计和指针
        alen += full_block_size;
        buf = (char *)buf + full_block_size;
