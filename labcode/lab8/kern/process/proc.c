...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2311369 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
...
void proc_run(struct proc_struct *proc)
{
    // LAB4:填写你在lab4中实现的代码
        ...
        */
    //LAB8 2313314 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
    ...
    {
        bool flags=0;
        struct proc_struct*pre=current,*next=proc;
        local_intr_save(flags);
        current=proc;
        lsatp(proc->pgdir);//修改satp寄存器中的值
        flush_tlb();//刷新tlb
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2311369  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
    ...
    if(proc==NULL){
     ...
     proc->parent = current;       // 子进程的父进程指向当前进程
...
{
     /* LAB8:EXERCISE2 2311656  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    // (2) 创建新的页目录表，并设置mm->pgdir为页目录表的内核虚拟地址
    if (setup_pgdir(mm) != 0)
    {
        goto bad_pgdir_cleanup_mm;
    }
    ...
    for (; ph < ph_end; ph++)
    {
        ...
        unsigned char *buf = kmalloc(PGSIZE);
        if (buf == NULL)
        {
            ret = -E_NO_MEM;
            goto bad_cleanup_mmap;
        ...
        while (start < end)
        {
            ...
            if ((ret = load_icode_read(fd, buf, size, ph->p_offset + (start - ph->p_va))) != 0)
            {
                kfree(buf);
                goto bad_cleanup_mmap;
            }
   ...
   uint32_t total_string_len = 0;
    for (int i = 0; i < argc; i++) {
        total_string_len += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
    }

    // 计算 argv 指针数组所需空间
    uint32_t uargv_size = argc * sizeof(char *);

    // 计算总共需要减去的栈空间，并进行对齐 
    uintptr_t stack_needed = sizeof(long) + uargv_size + total_string_len;
    uintptr_t current_sp = USTACKTOP - stack_needed;
    current_sp -= (current_sp % 16); // 强制对齐到16字节

    //定义三个区域的起始地址
    int *uargc_ptr = (int *)current_sp;
    char **uargv_ptr = (char **)(current_sp + sizeof(long));
    char *ustr_ptr = (char *)(current_sp + sizeof(long) + uargv_size);

    //填充内容
    *uargc_ptr = argc; // 压入 argc
    
    uint32_t offset = 0;
    for (int i = 0; i < argc; i++) {
        uargv_ptr[i] = ustr_ptr + offset; // 设置 uargv[i] 指向用户栈上的字符串
        strcpy(uargv_ptr[i], kargv[i]);   // 拷贝字符串到用户栈
        offset += strlen(kargv[i]) + 1;
    }
...
kernel_execve(const char *name, const char **argv)
{
    ...
    asm volatile(
        "mv sp, %0\n"//将内核栈指针（sp）设置为新中断帧（new_tf）的起始地址。
        "j __trapret\n"//从内核态转到用户态
        :
        : "r"(new_tf)//选择空闲的通用寄存器，将 new_tf 的值加载到这个寄存器中。
        : "memory");//确保汇编执行前，所有对 new_tf、current->tf 等内存的修改都已完成
    return ret;
}
